...
typedef struct _FILENAME {
	...
	int cslth;
}FILENAME;	//FILENAME : struct which store .obj file name and length of Control Section
...
typedef struct _bp {
	int breakpoint;
	struct _bp* next;
}bp;	//bp : struct which store breakpoint information 
bp* bp_head;	//bp_head is pointing head of bp linked list
bp* bp_prev;	//bp_prev is pointing previous node of bp

typedef struct _nixbpe {
	int ni;
	int xbpe;
}nixbpe;			//nixbpe : struct which store information aboue nixbpe 
nixbpe nixbpe_flag;	//flag which store memory's nixbpe info
...
int execaddr;	//variable which store EXECADDR
int estab_number;	//variable which stores total number of ESTAB element
char str_progaddr[31];	//arr which stores progaddr
char line_obj[270];		//arr which stores one line of obj file
char modi_value[7] = { 0 };	//arr which stores value which need to be modified
int* reference_number; //dinamic memory which stores ESTAB Reference number
int REG[13];		//array which stores information about REGISTER
int TA;	//Target Address at run
int format4_flag = 0;		//flag: 0-> format3, 1->format4
int run_end_flag = 0;		//flag : 0->run is not end, 1->run is end
int bp_zero = 0;		//if bp is set in 0, make it break and becomes 1
...
void command_progaddr()
{
	...
	//change string into hexa number
	progaddr = strtol(str_progaddr, NULL, 16);
	csaddr = progaddr;	//store progaddr into csaddr
	REG[8] = progaddr;	//store progaddr into PC REGISTER
}
...
void command_loader()
{
	//reset obj_name array and obj_name_size if this is not first time input about loader command
		...
		if (fp_obj == NULL)
		{
			...
			while (1) {
				...
				if (fgets(line_obj, 270, fp_obj) == NULL)
					break;
				//distinct line by calling pass1_line_distinct function
	...
	{
		FILE* fp_obj = fopen(obj_name[i].str_filename, "r");
		//Each Obj file opens, make dinamic memory for reference number
		reference_number = (int*)malloc(sizeof(int) * (estab_number+1));
			while (1) {
			//read 1 line
			if (fgets(line_obj, 270, fp_obj) == NULL)
				break;
			//distinct line by calling pass2_line_distinct function
	...
	//initialize Register value, when program loaded
	for(int i=0; i<13; i++)
	{
		//PC Register
		if(i==8)
...
/* purpose : distinct one line of obj file and  */
/*				store needed information		*/
/* return  : none                               */
/*----------------------------------------------*/
void pass1_line_distinct(char* one_line_obj, int obj_index)
{
	int line_index = 1;		//index of one_line_obj
	int record_flag = 0;	//flag : 1-> Head Record, 2-> Define Record, 3->End Record
	char _name[7] = { 0 };	//arr which stores program or symbol name
	char _addr[7] = { 0 };		//arr which stores program or symbol address
	int str_index = 0;		// index of _name or _addr
	if (one_line_obj[0] == 'H')
		record_flag = 1;
	...
	if (record_flag == 1)
	{
		char _length[7] = { 0 };		//arr which stores length of program
		...
		//store into the ESTAB by linked list
		ESTAB* es_node = (ESTAB*)malloc(sizeof(ESTAB));
		strcpy(es_node->control_section, _name);		//store control section name
		es_node->addr = strtol(_addr, NULL, 16) + csaddr;	//add CSADDR to get the address
		es_node->cslth = strtol(_length, NULL, 16);		//store CSLTH for the head symbol
		es_node->symbol_name[0] = '\0';	//store impossible file name char
		es_node->next = NULL;
		if (estab_head == NULL)
		{
			estab_head = es_node;
		...
		{
			//If same name in the ESTAB, error message
	...
	{ 
		while (one_line_obj[line_index] != '\n') {
			int six_name = line_index + 5;	//variable which stores index of six char after line_index
			int six_addr = line_index + 11; //variable which stores index of eleven char after line_index

			//store External Symbol Name
			...
			//store Relative address of symbol
			...
			es_node->addr = strtol(_addr, NULL, 16) + csaddr;	//add CSADDR to get the address
			es_node->control_section[0] = '\0';	//store impossible file name char
			es_node->next = NULL;
			if (estab_head == NULL)
			{
				estab_head = es_node;
	...
	//End Record, CSLTH is added to CSADDR
...
/* purpose : print load map					*/
/* return  : none                           */
/*------------------------------------------*/
void print_loadmap()
{
	int total_length = 0;	//variable which stores total length
	...
	//store total program length in the L REGISTER
	REG[2] = total_length;
...
/* purpose : free all ESTAB      									  */
/* return  : none                         							  */
/*--------------------------------------------------------------------*/
void free_estab()
{
	//if ESTAB's linked list exists, free every memory
	if (estab_head)
	{
		ESTAB* cur = estab_head;	//cur points current node
		while (1)
		{
			if (cur == NULL)
				break;
			ESTAB* prev = cur;	//prev is used to free node
			cur = cur->next;
			free(prev);
		}
		estab_head = NULL;
...
/* purpose : distinct one line at pass2		*/
/* return  : none                           */
/*------------------------------------------*/
void pass2_line_distinct(char* one_line_obj, int obj_index)
{
	...
	char _addr[7] = { 0 };		//arr which stores program or symbol address
	char modi_symbol[31] = { 0 };	//arr which stores symbol in modification line
	int str_index = 0;		// index of _name or _addr
	int dump_index = 0;	//index of virtual memory
	if (one_line_obj[0] == 'H')
		record_flag = 0;
	...
	if(record_flag == 0)
	{
		//store Program Name Reference Number 01
		for (line_index; line_index <= 6; line_index++)
		{
			if (one_line_obj[line_index] == ' ')
				continue;
		...
		//Store Program Name's PROGADDR into reference_number array
		reference_number[1] = find_estab(modi_symbol);
	}
	//Reference Record
	else if(record_flag == 4)
	{
		int out_flag = 0;	//flag to detect end of line
		//Save All Reference Number info through under while statement
		while (!out_flag) {
			char _length[3] = { 0 };		//arr which stores reference number 
			
			int i = line_index, end_i = line_index +2;		//variable to detect index of line
			
			//store reference number information
			for (i; i < end_i; i++)
			...
			end_i = i + 6
					;
			//detect end of Reference Record
			for (i; i < end_i; i++)
			{
				if (one_line_obj[i] == ' ')
					continue;
			...
			//Store Symbol's ADDR into reference_number array
			reference_number[refer_index] = find_estab(modi_symbol);

			memset(modi_symbol, 0, sizeof(modi_symbol));	//initalize modi_symbol array
			str_index = 0;
			line_index += 8;

			//If the last define record symbol is 6 words, check if the next is \n
			if (one_line_obj[line_index] == '\n')
				out_flag = 1;
		}
	}
	...
	{
		int text_length=0;	//variable which stores length of text line
		char _length[3] = { 0 };		//arr which stores length of text line
		char obj_value[3] = { 0 };		//arr which stores one byte of text line
		//store Text Address
		for (line_index; line_index <= 6; line_index++)
		{
			if (one_line_obj[line_index] == ' ')
		...
		//convert start addr and length into hexa number
		dump_index = strtol(_addr, NULL, 16);
		text_length = strtol(_length, NULL, 16);

		int format_index = 0; 	//variable which stores how many characters after line_index
		for(int cnt_obj=0; cnt_obj < text_length ; )
		{
			//Format 4, Need Reclocation, read 8 characters
			if(one_line_obj[line_index+2] == '1')
				format_index = line_index + 8;
			//read 6 characters
			else
				format_index = line_index + 6;	//variable which stores 6 after line_index
			for (int i = line_index; i < format_index;)
			{
				//save one byte value of text record into obj_value
				for (int j = i; j < i + 2; j++)
					obj_value[j - i] = one_line_obj[line_index++];
			
				//store obj_value into the virtual memory
	...
	{
		int modi_addr=0, modi_num = 0;	//variable which stores address of modification and how many half byte to be modified
		int arith_flag = 0;		//flag : 0 -> +(add) 1 -> -(minus)
		char _length[3] = { 0 };		//arr which stores length of program
		memset(modi_value, 0, sizeof(modi_value));
		...
		//Store Symbol's reference number in the modi_symbol
		while (one_line_obj[line_index] != '\n')
			modi_symbol[str_index++] = one_line_obj[line_index++];


		//convert addr and number into hexa number
		modi_addr = strtol(_addr, NULL, 16);
		modi_num = strtol(_length, NULL, 16);
		modi_addr += csaddr;
		
		//Store Value which need to be modified into string
		for (int i = 0; i < 3; i++)
		{
			int divide_modi = 0;	//variable to detect 0~9, or A~F ASCII CODE
		...
		//Change reference number string into the integer
		int refer_index = (int)strtol(modi_symbol , NULL, 16);

		//Calulate modification Record through callling calculate_modi
		calulate_modi(modi_value, modi_num, reference_number[refer_index], arith_flag);

		for (int i = 0; i < 3; i++)
		{
			char byte_value[2] = { 0 };	//array which store one byte of modified value

			//save one byte value of modified value into byte_value
			for (int j = i*2; j < i*2 + 2; j++)
				byte_value[j - i*2] = modi_value[j];

			//store byte_value into the virtual memory
			memory[modi_addr + i] = (unsigned char)strtol(byte_value, NULL, 16);
		}

	}
	//End Record is read, add CSLTH to CSADDR
	else if (record_flag == 3)
	{
		//If Address of First executable instruction exists, store it's information
...
/* purpose : find symbol in the ESTAB    */
...
/* purpose : add or minus symbol to the   */
/*                modification  value     */
/* return  : symbol's address             */
/*----------------------------------------*/
void calulate_modi(char* operand, int change_num, int estab_symbol, int arith_flag) {
	
	char* temp_modi_operand;		//array which store modified value
	int oper = 0;			//variable which stores integer of operand
	int temp_modi_index = 0;	//index of temp_modi_operand
	//Format 4, change only 5 half bytes. shift to left. 
	if (change_num == 5)
	{
		for (int i = 0; i < 5; i++)
	...
	//Make dinamic memory to store modi value 
	temp_modi_operand = (char*)malloc(sizeof(char) * (change_num+1));
	
	//if it is positive, change operand into integer to calculate
	if (operand[0] < '8')
	...
	//If oper is negative, becomes 8 word string
	if (oper < 0)
	{
		temp_modi_operand = (char*)realloc(temp_modi_operand, 9 * sizeof(int));
		//Change oper into 8 word string and store into temp_modi_operand
		sprintf(temp_modi_operand, "%X", oper);

		//if oper is negative, it becomes 8 words string. so need to change. 
		int temp_modi_oper_index = 2;
		if (change_num == 5)
			temp_modi_oper_index = 3;
		
		//put changed value in to the operand array through under for statement
		for (int i = temp_modi_oper_index; i < 8; i++)
		{
			char te[2];	//array which stores temporary value
		...
		if (change_num == 5)
		{
			//Change oper into 5 word string and store into temp_modi_operand
			sprintf(temp_modi_operand, "%5X", oper);
			strcat(operand, "1");
		}
		else {
			//Change oper into 6 word string and store into temp_modi_operand
			sprintf(temp_modi_operand, "%6X", oper);
		}

		//Add 0, if there is blank
		while (temp_modi_operand[temp_modi_index] == ' ')
			temp_modi_operand[temp_modi_index++] = '0';

		//Change the final result to the operand
...
/* purpose : change string into negative  */
/*   	     integer value   	          */
/* return  : integer value of string      */
/*----------------------------------------*/
int string_to_negin(char* operand, int length)
{
	char *binary_operand= (char*)calloc(length*4, sizeof(char));		//array which stores binary value of operand
	int result = 0, two = 1;	//variable which store result value of operand, two is for the pow of 2	
	for (int i = 0; i < length; i++)
	{
		//change each word into binary number and XOR through under switch statement (2's complement)
		switch (operand[i]) {
		case 'F':
	...
	result += 1;
	//return negative value of result
	return (-1) * result;
}

/*------------------------------------------*/
/* func_name : print_bp                     */
/* purpose : print all breakpoint			*/
/* return  : none                           */
/*------------------------------------------*/
void print_bp()
{
	//If bp linked list exists, print all breakpoints
	if (bp_head) {
		printf("\t     breakpoint\n");
		printf("\t     ----------\n");
		bp* cur = bp_head;
	...
	//If bp linked list not exist, print error message
	else
	{
...
/* purpose : get number after bp command and store in to the bp linked list			*/
/* return  : none                           										*/
/*----------------------------------------------------------------------------------*/
void command_bp()
{
	char temp_bp_str[7] = { 0 };	 //array which store string after bp and blank
	int temp_bp_size = 0, bp_index = 3, bp_num=0;	//variable which stores size of string after bp, index of temp_bp_str, number after bp command
	
	//if bp blank and nothing entered, print wrong 
	if(command[3]=='\0')
	{
		error_flag = 1;
	...
	{
		//if too much value is in input, print error message
	...
	//if clear entered after bp command
	if (!strncmp(temp_bp_str, "clear", 5))
		free_bp(1);
	else {
		//check temp_bp_str is valid address character under for statement
		for (int i = 0; i < temp_bp_size; i++)
		{
			//if char is not valid address, print error message
		...
		//if bp_num is inside program's memory address bound, make linked list
		...
		//if bp_num is not in program's memory address bound, print error message
		else
		{
			error_flag = 1;
...
/*--------------------------------------------------------------------*/
/* func_name : free_bp        										  */
/* purpose : free all bp linked list      							  */
/* return  : none                         							  */
/*--------------------------------------------------------------------*/
void free_bp(int clear_flag)
{
	//if bp's linked list exists, free every memory
	if (bp_head)
	{
		bp* cur = bp_head;	//cur points current node
		while (1)
		{
			if (cur == NULL)
				break;
			bp* previous = cur;	//previous is used to free node
			cur = cur->next;
			free(previous);
		}
		//if this function called in clear command, print under statement
		if(clear_flag)
	...
	{
		//if thins function called in clear command, print under statement
		if(clear_flag)	
			printf("\t     Error! : No existing breakpoints!\n");
		return;
	}
...
/* purpose : check bp's addr if it is valid address       			  */
/* return  : //if c is valid address char, return 1, else 0           */
/*--------------------------------------------------------------------*/
int check_bp_addr(char c)
{
	//if c is valid address char, return 1
	if (c >= '0' && c <= '9')
		return 1;
	else if (c >= 'A' && c <= 'F')
		return 1;
...
void command_run()
{
	char opcode, xbpe_flag;		//variable which stores opcode and xbpe flag setting information 
	int opcode_val = 0;	//variable which stores integer opcode value
	int first_run = 0;		//variable if run first time,  make program go through under while statement
	/* Register Number : 
		A : 0
		X : 1
		L : 2
	...
	while (1) {
		...
		//If breakpoint, and not first time running, break
		if (first_run != 0 && find_pc_bp() == 1)
			break;
		
		//If breakpoint, and breakpoint is 0, break
		...
		//To detect if it is first run or not
		if (first_run == 0)
			first_run = 1;
		...
		//Get opcode value by AND with FC
		opcode_val = ((int)opcode & 0xFC);

		//Get ni value by AND with 03
		...
		{
			int m_m2 = get_TA_3();		//variable which stores memory value

			//compare between A , m...m+2
		...
		{
			int temp_memory = get_TA_3();	//variable which stores memroy to compare 
			REG[1] += 1;
	...
	if (run_end_flag)
	{
		...
		//initialize PC, L register
		REG[8]=0;
		REG[2]=0;
		run_end_flag = 0;
	}
	//if stop at checkpoint
	else
...
/* purpose : if current pc is same as	    */
/*	bp in the bp linked list, return 1		*/
/* return  : 1 if current pc == bp, else 0  */
/*------------------------------------------*/
int find_pc_bp()
{
	//If bp linked list exists, find if current pc is same as bp
	if (bp_head) {
		bp* cur = bp_head;
		while (cur != NULL)
		{
			//if pc is same as bp in the linked list, return 1
			if (cur->breakpoint == REG[8])
				return 1;
			cur = cur->next;
		}
	}
	//if there is no bp linked list, no bp exists, so always return 0
	return 0;
}

/*------------------------------------------------------------------*/
/* func_name : get_TA_3        										*/
/* purpose :  distinct nixbpe and get target address for format 3/4	*/
...
int get_TA_3()
{
	...
	if (nixbpe_flag.ni == 0)
	{
		//SIC -> bpe disp/address are all address.
	...
	//n is not 0, i is not 0 so can be format4
	else {
		//format 4
		if ( (nixbpe_flag.xbpe & 0x01) == 1)
			format4_flag = 1;

		//Get memory address by format 3 or 4
		if (format4_flag)
		{
			disp = ((int)memory[REG[8] + 1] % 16) * 256 * 256 + (int)memory[REG[8] + 2] * 256 + (int)memory[REG[8] + 3];

			//if negative, change to positive by 2's complement
			if ((disp & 0x80000) != 0)
			{
				disp = ~disp;
				disp += 1;
...
/*---------------------------------------------------------------*/
/* func_name : get_TA_2        									 */
/* purpose :  distinct nixbpe and get target address for format2 */
/* return  : none                        						 */
/*---------------------------------------------------------------*/
void get_TA_2()
{
	...
	//store register number in the r1, r2
	REG[10] = ((int)memory[REG[8] + 1] & 0xf0) >> 4;
	REG[11] = ((int)memory[REG[8] + 1] & 0x0f);

	////store register value in the r1, r2
	//REG[10] = REG[REG[10]];
	//REG[11] = REG[REG[11]];
